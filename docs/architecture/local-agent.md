...
- Launch profiles define executable, argument templates, working-directory policy, environment allowlist, and a local tool adapter. The optional `tool_adapter` currently defaults to the only supported value, `generic`; unknown values fail configuration validation. No remote client supplies an unchecked executable path, adapter, or environment.
