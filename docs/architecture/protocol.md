...
- If both peers advertise `attachment.transfer_v1`, they must also advertise `terminal.sequenced_io_v1` and `control.error_v1`. Attachment begin/chunk/complete/cancel/discard messages are accepted only on that negotiated ordered stream. `AttachmentCancel` abandons one active partial without removing a completed file; `AttachmentDiscard` validates 1–10 unique opaque IDs before mutation, then idempotently removes active partials and completed Agent-generated files and records `CANCELLED` tombstones. The browser keeps one attachment operation in flight and waits until the Agent cumulative acknowledgement covers its sequence before advancing; a UI abort never drops an already-sent operation. Chunk acknowledgements drive progress, and an acknowledged complete means verification/publication committed. A rejected operation is not committed, returns `ATTACHMENT_TRANSFER_FAILED` with the last committed sequence acknowledged, and abandons its active partial. On selection failure or abort, the client sends one discard containing every ID whose begin was attempted, including completed files. If a physical connection closes before acknowledgement, a same-PTY rebind sends `AttachmentTransferStatusRequest` for only the opaque transfer ID. The Agent reports `UNKNOWN`, `ACTIVE`, `COMPLETED`, or `CANCELLED`; only `ACTIVE` carries the committed `next_offset_bytes` cursor. The client resolves or replays the exact pending operation only when that state proves it safe and otherwise fails closed. For a lost discard acknowledgement it queries each ID: all `CANCELLED`/`UNKNOWN` results resolve the discard, while any `ACTIVE`/`COMPLETED` result replays the whole discard. Status is session-local and read-only, exposes no attachment metadata or path, and cannot create staging or a transfer manager.
