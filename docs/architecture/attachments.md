...
13. Cleanup policy removes the files at session end or explicit user action.
...
While the rebound session ID and generation still identify the same PTY, the browser preserves the prompt action ID and phase across physical WebSocket reconnects, resends prepare/commit/cancel only after the new negotiated stream is bound, and waits for the matching preview or cumulative acknowledgement. A different PTY identity abandons the old action and staged transfer IDs instead of replaying session-local references. A lost commit acknowledgement is therefore safe to retry against the Agent's exactly-once registry. A terminal `ProcessExit` first honors any cumulative acknowledgement that already confirms a pending commit or cancel; otherwise it rejects pending browser work without reconnecting an ended session. Transfer cancellation remains best effort: cancelling while an operation awaits acknowledgement closes the connection to avoid reusing uncertain sequence state, and completed files remain staged until session cleanup. A disconnect before a transfer acknowledgement can still leave the client unable to determine whether the Agent committed that operation; outcome reconciliation and chunk resume are not implemented.

The production Agent Hello and browser Client Hello still do not advertise either attachment capability, and the App feature flag remains off. Explicit multi-file transfer cancellation semantics, per-device/Agent aggregate limits, crash recovery scans, no-workspace sandbox staging, and end-to-end/real-device validation remain before advertisement is enabled.
