...
- Protocol V1 now defines additive `AttachmentBegin`, `AttachmentChunk`, `AttachmentComplete`, and `AttachmentCancel` envelopes with opaque transfer IDs, byte offsets, per-chunk SHA-256, and total SHA-256 metadata. The non-advertised `attachment.transfer_v1` capability requires ordered `terminal.sequenced_io_v1` and `control.error_v1`; shared Go/TypeScript generation and an attachment-begin golden vector protect the wire contract while both peers keep the feature disabled until protocol dispatch, session ownership, and the client flow are complete.
...
- The session-side transfer manager now validates opaque IDs and display metadata, reserves per-file/session quota, enforces exact ordered chunks with per-chunk SHA-256, verifies total size and SHA-256, checks a deterministic MIME/extension allowlist against binary headers or whole-stream UTF-8 text validation, publishes only verified files, and provides idempotent completion/cancellation plus active-partial cleanup on close. Completed files remain quota-reserved until session cleanup.
- Protocol V1 now decodes the four attachment operations behind the peer capability declaration and dispatches them to one lazily created, session-owned manager for workspace sessions. Manager side effects precede sequence commit and acknowledgement; a rejection leaves the failed sequence uncommitted and reports only `ATTACHMENT_TRANSFER_FAILED`. Session exit closes the manager before staging cleanup.
- A dark, capability-gated browser flow now provides allowlisted file and camera input, image preview and review, explicit confirmation, SHA-256 metadata, checksummed chunks capped at 48 KiB and reduced under smaller negotiated envelopes, aggregate WebSocket queue progress, best-effort cancellation, and recoverable local errors. It remains disabled in the App and is not advertised by either production Hello. The client-side 10-file check is advisory until trusted prompt-action context exists. Acknowledgement-correlated backpressure/server-error handling, explicit multi-file cancellation semantics, incremental hashing, tool adapters, trusted enforcement, per-device/Agent aggregate limits, crash recovery scans, no-workspace sandbox staging, and end-to-end/real-device validation remain before the phase exit gate is met.
