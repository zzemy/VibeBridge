...
- Protocol V1 now decodes the four attachment operations behind the peer capability declaration and dispatches them to one lazily created, session-owned manager for workspace sessions. Manager side effects precede sequence commit and acknowledgement; a rejection leaves the failed sequence uncommitted and reports only `ATTACHMENT_TRANSFER_FAILED`. Session exit closes the manager before staging cleanup. The Agent and browser still do not advertise attachment transfer. Picker/progress/confirmation UX, tool adapters, trusted prompt-action context for the 10-file limit, per-device/Agent aggregate limits, crash recovery scans, and no-workspace sandbox staging remain before the phase exit gate is met.
