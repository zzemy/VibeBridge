...
- A dark, capability-gated browser flow now provides allowlisted file and camera input, image preview and review, explicit confirmation, SHA-256 metadata, 48 KiB checksummed chunks under the current envelope limit, aggregate progress, cancellation, and recoverable local errors. It remains disabled in the App and is not advertised by either production Hello. The client-side 10-file check is advisory until trusted prompt-action context exists. Tool adapters, trusted enforcement, per-device/Agent aggregate limits, crash recovery scans, no-workspace sandbox staging, and end-to-end/real-device validation remain before the phase exit gate is met.
