...
- Negotiated `terminal.resize_end_v1` now carries bounded `TerminalResize` and explicit `EndSession` controls in the ordered Protobuf stream. Peers without the capability retain JSON resize/end compatibility, while negotiated peers reject those JSON controls. Stable V1 process-exit, error, and ping/pong messages, previous-client compatibility coverage, and a removable legacy boundary remain in progress.
