...
- A dark, capability-gated browser flow now provides allowlisted file and camera input, image preview and review, explicit confirmation, incremental SHA-256 over bounded 1 MiB reads, and checksummed chunks capped at 48 KiB and reduced under smaller negotiated envelopes. One attachment operation remains in flight until the Agent cumulative acknowledgement covers its sequence, so progress represents acknowledged chunks and success follows acknowledged complete/verification. Explicit `ATTACHMENT_TRANSFER_FAILED` correlation abandons the partial, rebuilds the physical connection, and requires a whole-file retry. It remains disabled in the App and is not advertised by either production Hello. The client-side 10-file check is advisory until trusted prompt-action context exists. Pre-acknowledgement disconnect outcome reconciliation, explicit multi-file cancellation semantics, tool adapters, trusted enforcement, per-device/Agent aggregate limits, crash recovery scans, no-workspace sandbox staging, and end-to-end/real-device validation remain before the phase exit gate is met.
