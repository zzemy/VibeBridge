...
PTY output is sent to the browser as binary WebSocket frames so terminal bytes and ANSI sequences are preserved. When both peers negotiate `terminal.sequenced_io_v1`, terminal input/output and acknowledgements are protobuf envelopes with connection-local ordering. When they also negotiate `session.resume_v1`, `AttachSession` and `SessionStatus` bind each physical connection to a resumable PTY generation. Negotiated `terminal.resize_end_v1` adds ordered resize and explicit end controls; older peers retain their JSON adapters. Negotiated `session.process_exit_v1` adds an ordered final process outcome without exposing host errors; peers without it retain the JSON exit notification. Otherwise the staged legacy path uses raw binary output and JSON input. Health and error controls currently use JSON text frames in both paths.
