...
func run(args []string) error {
	...
	if *diagnose {
		return runDiagnostics(*addr, keyPath, issuer.PublicKey())
	}
...
func loadOrCreateIssuerKey(path string, allowOverwrite bool) (ed25519.PrivateKey, error) {
	...
	if !allowOverwrite {
		// We will create the file; ensure the parent directory exists
		// so the WriteFile below does not fail for the wrong reason.
	}
...
func runDiagnostics(addr, keyPath string, public ed25519.PublicKey) error {
