...
test("restores a draft for the active browser session", async () => {
  ...
  const { unmount } = render(<PromptComposer disabled={false} storageKey="draft-key" onSubmit={vi.fn()} />);

  await user.type(screen.getByRole("textbox"), "keep this draft");
  expect(sessionStorage.getItem("draft-key")).toBe("keep this draft");

  unmount();
  render(<PromptComposer disabled={false} storageKey="draft-key" onSubmit={vi.fn()} />);
...
test("submits without Enter in insert-only mode", async () => {
  ...
  render(<PromptComposer disabled={false} storageKey="draft-key" onSubmit={onSubmit} />);
