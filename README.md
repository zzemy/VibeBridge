...
Each workspace-bound PTY session reserves `.vibebridge/uploads/<session-id>/` for temporary attachment staging and removes that session directory after the PTY ends or fails to launch when cleanup succeeds; a cleanup failure retains the boundary and emits a privacy-safe diagnostic event. Physical names come from an Agent-generated opaque ID, and link escapes are rejected. Protocol V1 includes begin/chunk/complete/cancel attachment envelopes behind `attachment.transfer_v1`; the Agent can dispatch declared transfers to a session-owned manager that enforces quota, ordered chunks, checksums, content policy, cancellation, and cleanup without committing rejected client sequences. A dark, capability-gated browser flow now covers allowlisted file/camera selection, preview, confirmation, bounded checksummed chunks, progress, cancellation, and client-side recovery. The production Agent and browser still do not advertise the capability, so attachments remain unavailable until trusted prompt-action enforcement and tool adapters are complete. Add `/.vibebridge/` to every registered repository's ignore rules; this repository already includes it.
