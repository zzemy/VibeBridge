...
Each workspace-bound PTY session reserves `.vibebridge/uploads/<session-id>/` for temporary attachment staging and removes that session directory after the PTY ends or fails to launch when cleanup succeeds; a cleanup failure retains the boundary and emits a privacy-safe diagnostic event. Physical names come from an Agent-generated opaque ID, and link escapes are rejected. Protocol V1 includes begin/chunk/complete/cancel attachment envelopes behind `attachment.transfer_v1`; the Agent dispatches declared transfers to a session-owned manager that enforces quota, ordered chunks, checksums, content policy, cancellation, and cleanup. A rejected operation is not committed and its active partial is abandoned. A dark, capability-gated browser flow now covers allowlisted file/camera selection, preview, confirmation, and bounded checksummed chunks with one operation in flight; Agent cumulative acknowledgements drive progress, and success follows acknowledged verification/publication. Explicit rejection rebuilds the physical connection and requires a whole-file retry. A disconnect before acknowledgement still has an unresolved outcome, and multi-file cancellation, trusted prompt-action enforcement, and tool adapters remain incomplete. The production Agent and browser therefore still do not advertise the capability. Add `/.vibebridge/` to every registered repository's ignore rules; this repository already includes it.
