...
Each workspace-bound PTY session reserves `.vibebridge/uploads/<session-id>/` for temporary attachment staging and removes that session directory when the PTY ends or fails to launch. Physical names come from an Agent-generated opaque ID, and link escapes are rejected. Protocol V1 includes begin/chunk/complete/cancel attachment envelopes behind `attachment.transfer_v1`, but neither peer advertises that capability until the transfer state machine and integrity checks are implemented, so attachment transfer is not exposed yet. Add `/.vibebridge/` to every registered repository's ignore rules; this repository already includes it.
